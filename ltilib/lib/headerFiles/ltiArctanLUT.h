 ...
 * creation ...: 01.06.2003
 * revisions ..: $Id: ltiArctanLUT.h,v 1.2 2004/01/06 18:02:00 sltilib Exp $
 */
...
namespace lti {
  /**
   * Following class provides a mono-state object that contains a 1MB Look-up
   * Table to accelerate the computation of the arc-tangens.
   *
  ...
  inline float arctanLUT::operator() (const double& dy,const double& dx) const {
  ...
  inline float arctanLUT::operator() (const float dy,const float dx) const {
    if (dy < 0.0f) {
      if (dx < 0.0f) {
        if (dx<dy) {
          return atanLUT[iround(-255.0f*dy/dx)][-255];
        } else {
          return atanLUT[-255][iround(-255.0f*dx/dy)];
        }
      } else {
        // dy < 0 and dx >= 0
        if (dx > -dy) {
          return atanLUT[iround(dy*255.0f/dx)][255];
        } else {
          return atanLUT[-255][iround(-255.0f*dx/dy)];
        }
      }
      ...
      if (dx < 0.0f) {
        ...
        if (-dx > dy) {
          ...
          return atanLUT[255][iround(dx*255.0f/dy)];
        }
      } else {
        // dy >= 0 and dx >= 0
        if (dx>dy) {
