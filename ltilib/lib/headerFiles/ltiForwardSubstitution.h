 ...
 * file .......: ltiForwardSubstitution.h
 * authors ....: Peter Doerfler
 * organization: LTI, RWTH Aachen
 * creation ...: 20.5.2003
 * revisions ..: $Id: ltiForwardSubstitution.h,v 1.7 2005/06/16 12:37:34 doerfler Exp $
 */

#ifndef _LTI_FORWARD_SUBSTITUTION_H_
#define _LTI_FORWARD_SUBSTITUTION_H_
...
namespace lti {
  /**
   * Forward substitution. Let \a L be a lower triangular matrix and
   * \a U an upper triangular matrix. Forward substitution solves the
   * following linear equation systems for \a x:
   *
   * L*x=b or x*U=b
   *
   * In the first case, \a x and \a b are column vectors, in the latter
   * case they are row vectors. The data %matrix can be given to the
   * functor via the parameters or directly in the apply
   * methods. There are also apply methods that calculate the forward
   * substitution for multiple right-sides \a b. The vectors are
   ...
   */
  template<class T>
  class forwardSubstitution : public linearAlgebraFunctor {
  public:
    /**
    ...
    public:
      ...
      /**
       * default constructor. The triangularType is Lower by default.
       */
      ...
      const char* getTypeName() const {
        return "forwardSubstitution::parameters";
      };
...
#     endif
     ...
     */
    forwardSubstitution(const matrix<T>& theMatrix, 
    ...
    bool apply(const matrix<T>& theMatrix, const vector<T>& b,
               vector<T>& x, const typename parameters::eTriangularType& tType) const;
    ...
    bool apply(const matrix<T>& theMatrix, const matrix<T>& b, 
               matrix<T>& x, const typename parameters::eTriangularType& tType) const;
