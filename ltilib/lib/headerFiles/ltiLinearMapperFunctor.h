 ...
 * file .......: ltiLinearMapperFunctor.h
 * authors ....: Pablo Alvarado
 * organization: LTI, RWTH Aachen
 * creation ...: 10.7.2000
 * revisions ..: $Id: ltiLinearMapperFunctor.h,v 1.13 2005/02/02 14:16:23 doerfler Exp $
 */

#ifndef _LTI_LINEAR_MAPPER_FUNCTOR_H_
#define _LTI_LINEAR_MAPPER_FUNCTOR_H_
...
namespace lti {
   ...
   * output point will be also outer bounds
   *
   ...
   *   // the mapper functor
   *   lti::linearMapperFunctor<lti::channel::value_type> mapper;
   *   lti::linearMapperFunctor<lti::channel::value_type>::parameters mapParam;
   *
   *   mapParam.generateFrom(histo);
   *   mapParam.lowerLimitsInput.fill(0.0f);
   *   mapParam.upperLimitsInput.fill(1.0f);
   *
   *   mapper.setParameters(mapParam);
   *
   *   lti::point p,q;
   *
   ...
   *       histo.put(mapper.apply(r.at(p),g.at(p),q));
   *     }
   *   }
   * \endcode
   */
  template <class Tin, class Tout=int>
    class linearMapperFunctor : public mapperFunctor<Tin,Tout> {
    public:
    /**
     * the parameters for the class linearMapperFunctor
     */
...
#     endif
     ...
     */
    linearMapperFunctor(const linearMapperFunctor& other);

    /**
     * destructor
     */
    virtual ~linearMapperFunctor();

    /**
     * returns the name of this type ("linearMapperFunctor")
     */
     ...
     * integer vectorm, both coded as a points
     * @param src the two dimensional point representing the input vector
     * @param dest point where the result will be left.
     * @result a reference to the <code>dest</code>.
     */
     ...
     */
    linearMapperFunctor& copy(const linearMapperFunctor& other);
...
#include "ltiLinearMapperFunctor_template.h"
