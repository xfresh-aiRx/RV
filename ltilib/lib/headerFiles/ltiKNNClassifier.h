 ...
 * creation ...: 15.06.2003
 * revisions ..: $Id: ltiKNNClassifier.h,v 1.15 2005/06/16 09:32:40 doerfler Exp $
 */
...
namespace lti {
   ...
   *
   * lti::kNNClassifier::outputVector outv; // here we will get some
...
#     endif
       ...
       *
       * You usually use this mode with kNN=1.  The normalization of the
     ...
     * the outputVector %object with the classification result.
     *
     ...
     *
     * Since no classifier::outputVector is constructed, only the
    ...
    inline int getColumnId(const int columnId) const {
      if (static_cast<uint32>(columnId)<static_cast<uint32>(nClasses)) {
