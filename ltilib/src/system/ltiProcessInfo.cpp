 ...
 * creation ...: 30.10.00
 * revisions ..: $Id: ltiProcessInfo.cpp,v 1.22 2005/06/22 15:42:10 doerfler Exp $
 */
...
# ifdef HAVE_PROC
  ...
  void processInfo::getCurrentCPUStatus() {
    ...
    while (!feof(f)) {
      fgets(workBuffer,bufLength,f); // read line (<=bufLength characters)
    ...
    if (f!=NULL) {
      ...
      fgets(workBuffer,bufLength,f); // read line
  ...
  void processInfo::getCurrentSystemStatus() {
    ...
    fgets(workBuffer,bufSize,f); // drop first line
    fgets(workBuffer,bufSize,f);
    // terminate string in case of buffer overflow
    workBuffer[bufSize-1]='\0';
    sscanf(workBuffer, "Mem: %d %d %d %d %d %d", &state.coremem, &dummy, &state.freecoremem, &dummy, &state.buffers, &state.cached);
    fgets(workBuffer,bufSize,f);
    // terminate string in case of buffer overflow
    workBuffer[bufSize-1]='\0';
