 ...
 * file .......: ltiSerialStatsExtFunctor.h
 * authors ....: Jochen Wickel,Pablo Alvarado
 * organization: LTI, RWTH Aachen
 * creation ...: 31.12.2002
 * revisions ..: $Id: ltiSerialStatsExtFunctor.h,v 1.9 2005/06/16 12:37:35 doerfler Exp $
 */

#ifndef _LTI_SERIALSTATSEXT_FUNCTOR_H_
#define _LTI_SERIALSTATSEXT_FUNCTOR_H_
...
namespace lti {
  /**
   * Functor which computes several kinds of means for vectors and
   * matrices.  It also computes the extremes of the given data, which
   * is the main difference with his brother functor
   ...
   * \code
   *
   * lti::serialStatsExtFunctor<double> stats;
  ...
  class serialStatsExtFunctor : public statisticsFunctor {
  public:
    /**
    ...
    public:
      ...
      const char* getTypeName() const {
        return "serialStatsExtFunctor::parameters";
      };
...
#     endif
     ...
     * @param min the minimum value considered until now
     * @param max the maximum value considered until now
     */
     ...
     * @param mean the result, where the mean will be put
     * @param variance the result, where the variable will be put
     * @param min contains the minimum value for each dimension found in the
     *            data.
     * @param max contains the maximum value for each dimension found in the
     *            data.
     */
    void apply(vector<U>& mean, vector<U>& variance,
               vector<U>& min,  vector<U>& max) const;
    ...
    /**
     * Get the minimum of all vectors (minimum at each component)
     * considered so far.
     */
...
#include "ltiSerialStatsExtFunctor_template.h"
