 ...
 * creation ...: 18.3.2002
 * revisions ..: $Id: ltiUciDataReader.cpp,v 1.15 2005/06/16 12:37:11 doerfler Exp $
 */
          ...
          if (!sstr.eof()) {
            if (param.hasIds) {
              ...
              if (sstr >> tId) {
                ...
                ok = false;
                char err[100];
                sprintf(err,"Error in line %i",tIds.size()+1);
                setStatusString(err);
              }
            }
            if (ok) {
            ...
            b = false;
            char err[100];
            sprintf(err,"Error in line %i",tData.size()+1);
            setStatusString(err);        }
        }
      }
