 ...
 * creation ...: 8.5.2001
 * revisions ..: $Id: ltiUsePalette.h,v 1.18 2005/06/16 12:35:44 doerfler Exp $
 */
...
namespace lti {
   ...
   * - For a given region index map, which is usually an lti::imatrix or 
   *   a lti::channel8, and a palette, replace in a destination image
   *   each region label by the corresponding palette entry.  This is a simple
   *   but frequent operation when dealing with segmentation results, color
...
#     endif
      ...
      /**
       * The palette to be used.  This is the vector of rgbPixel
       * values to be used.
       *
    ...
    /**
     * returns the name of this type ("usePalette")
     */
    ...
    /**
     * Find for each pixel in the given image the best entry in the palette
     ...
     */
    const kdTree<rgbPixel,int>& getKdTree() const;
