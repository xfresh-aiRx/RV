 ...
 * file .......: ltiHomography9DofEstimator.h
 * authors ....: Claudia Goenner
 * organization: LTI, RWTH Aachen
 * creation ...: 14.4.2004
 * revisions ..: $Id: ltiHomography9DofEstimator.h,v 1.5 2005/06/16 12:35:10 doerfler Exp $
 */

#ifndef _LTI_HOMOGRAPHY9_DOF_ESTIMATOR_H_
#define _LTI_HOMOGRAPHY9_DOF_ESTIMATOR_H_
...
namespace lti {
   ...
   * between two images using an homogenous approach and least squares. It
   * estimates all 9 parameters of the homography by a svd-composition.
   */
  class homography9DofEstimator : public homographyEstimatorBase {
  public:
    /**
     * The parameters for the class homography9DofEstimator
     */
...
#     endif
     ...
     */
    homography9DofEstimator(const parameters& par);

    /**
     * Copy constructor
     * @param other the object to be copied
     */
    homography9DofEstimator(const homography9DofEstimator& other);

    /**
     * Destructor
     */
    virtual ~homography9DofEstimator();

    /**
     * Returns the name of this type ("homography9DofEstimator")
     */
  ...
  inline fpoint homography9DofEstimator
...
//     const float denom ( *(++it) * pt.x + *(++it) * pt.y + *(++it) );
    const float denom (transf.at(6)*pt.x + transf.at(7)*pt.y + transf.at(8));
    res.x /= denom;
    res.y /= denom;
  ...
  inline dpoint  homography9DofEstimator
...
//     const double denom ( *(++it) * pt.x + *(++it) * pt.y + *(++it) );
    const double denom (transf.at(6)*pt.x + transf.at(7)*pt.y + transf.at(8));
    res.x /= denom;
    res.y /= denom;
