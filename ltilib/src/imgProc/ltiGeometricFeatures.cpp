 ...
 * creation ...: 11.1.2001
 * revisions ..: $Id: ltiGeometricFeatures.cpp,v 1.50 2005/10/28 15:56:42 doerfler Exp $
 */
...
# endif
  ...
  bool geometricFeatures::apply(const lti::areaPoints& ap,
    ...
    if (mParams.boundaryDefinition == parameters::OuterBoundary) {
      ...
      if (bp.size() < 2) { // special case
        ...
        boundaryLength += outerBoundaryLength[dirBuf[1]][dirBuf[0]];
        // last-first-second
        ...
        for (; itBp != bp.end(); previous = itBp++) {
          ...
          boundaryLength +=
            outerBoundaryLength[dirBuf[dirIndex]][dirBuf[1-dirIndex]];
          dirIndex = 1 - dirIndex;
        }
      }
  ...
  float geometricFeatures::getLength(const borderPoints& bp,
    ...
    if (boundaryDefinition == parameters::CentralBoundary) {
      ...
      // special cases: butlast-last-first, and last-first-second.
      // treat these now:
      lti::point firstStep, lastStep; // first-second and butlast-last; last-first -> step
      step = bp.front() - bp.back();
      itBp = bp.begin();
      itBp++;
      firstStep = *itBp - bp.front();
      ...
      // now treat non-special cases; we start with the second step, i.e. second-third
      itBp = bp.begin();
      itBp++;
      previous = itBp++;
      for (; itBp != bp.end(); previous = itBp++) {
        step = *itBp - *previous;
        dirBuf[dirIndex] = direction[step.x+1][step.y+1];
        boundaryLength += outerBoundaryLength[dirBuf[dirIndex]][dirBuf[1-dirIndex]];
        dirIndex = 1 - dirIndex;
      }
      return static_cast<float>(boundaryLength);
    } else if (boundaryDefinition == geometricFeatures::parameters::Approximation) {
      float boundaryLength = getLength(bp,geometricFeatures::parameters::CentralBoundary);
      return boundaryLength == 0 ? 1 : boundaryLength; // treat special case of single pixel
