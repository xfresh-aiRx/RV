 ...
 * creation ...: 10.9.2003
 * revisions ..: $Id: ltiSelective2DConvolution.cpp,v 1.5 2005/06/16 12:35:44 doerfler Exp $
 */
...
# endif
    ...
    if (notNull(other.kernel2D)) {
      kernel2D = dynamic_cast<lti::kernel2D<float>*>(other.kernel2D->clone());
    }
  ...
  void selective2DConvolution::parameters::setKernel2D(const lti::kernel2D<float>& aKernel) {
    delete kernel2D;
    kernel2D = 0;
    kernel2D = dynamic_cast<lti::kernel2D<float>*>(aKernel.clone());
  }
